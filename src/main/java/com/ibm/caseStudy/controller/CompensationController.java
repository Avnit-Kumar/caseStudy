...
public class CompensationController {
	...
	@PostMapping("/add")
	public String addCompensation(@Valid @ModelAttribute("compensation") CompensationDTO dto, BindingResult result,
			Model model) {
	...
	@PostMapping("/history")
	public String viewHistory(@RequestParam Long employeeId, @RequestParam String startDate,
		...
		LocalDate end = LocalDate.parse(endDate + "-01")
				.withDayOfMonth(LocalDate.parse(endDate + "-01").lengthOfMonth());
		...
		List<MonthlyCompensationDTO> history = service.getMonthlyCompensationHistory(employeeId, start, end);
	...
	public String viewBreakdown(@PathVariable Long employeeId, @PathVariable String month, Model model) {

		YearMonth yearMonth = YearMonth.parse(month);

		List<CompensationDTO> breakdown = service.getCompensationBreakdown(employeeId, yearMonth);

		BigDecimal total = breakdown.stream().map(CompensationDTO::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
	...
	@GetMapping("/edit/{id}")
	public String showEditCompensationForm(@PathVariable Long id, Model model) {
	...
	@PostMapping("/edit/{id}")
	public String updateCompensation(@PathVariable Long id, @RequestParam BigDecimal amount,
		...
		try {
			...
			return "redirect:/compensation/breakdown/" + comp.getEmployee().getId() + "/"
